...
git log
git log --oneline
# head하단의 로그만 보이는 것이 아니라, 모든 commit 이력 조회
...
# pull은 원격변경사항을 local로 내려받는 것(fetch+mergy)
git pull origin main

# fetch는 변경사항을 local가져오되, 병합은 하지 않는 것
...
git tag 태그버전명(v1.1)
# git release : 코드의 commit, push오 상관없이 별개로 작업 진행 
...
# 새로운 작업시작 시에, main을 최신화 한 후에 main을 기준으로 branch 생성